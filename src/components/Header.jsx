...
import { IoArrowForwardOutline, IoSchoolOutline, IoRibbonOutline, IoFlaskOutline, IoBookOutline } from "react-icons/io5";
...
export default function Header() {
  ...
  return <motion.section className="hero" variants={pageStagger} initial="hidden" animate="show">
      ...
      <motion.div className="hero-bg" variants={fadeDown}>
        <video autoPlay loop muted playsInline className="hero-bg-image">
          <source src={HeroVideo} type="video/mp4" />
        </video>
      </motion.div>

      <motion.div className="hero-overlay" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} transition={{
      ...
      <div className="hero-container">
        <div className="hero-content">
          <motion.h1 className="hero-title-center" variants={fadeUp}>{t("science_based")}<br></br> <span style={{
            color: "#F9F8F3",
            WebkitTextFillColor: "#F9F8F3"
          }}>{t("practice_oriented")}</span>
          </motion.h1>

          <div className="hero-btn-group">
            <motion.button className="hero-btn" type="button" variants={btnPop} whileHover={{
            y: -2
          }} whileTap={{
            scale: 0.98
          }} transition={{
            duration: 0.18
          }} onClick={() => navigate('/')}>{t("apply_now_1")}<IoArrowForwardOutline />
            </motion.button>
          ...
          <motion.div className="hero-stack-carousel" variants={fadeUp}>
            <Swiper modules={[Autoplay]} spaceBetween={14} slidesPerView={1.2} breakpoints={{
            560: {
              slidesPerView: 2.2
            },
            760: {
          ...
          }} loop={true} speed={7000} // makes it smooth and continuous
          autoplay={{
            delay: 0,
            disableOnInteraction: false,
            pauseOnMouseEnter: false
          }} allowTouchMove={false} className="hero-swiper">
              {cards.map((c, i) => <SwiperSlide key={i}>
                  <button className={`hero-card ${c.tone}`} type="button" onClick={() => {
                if (c.link && c.link !== "#") {
                  navigate(c.link);
                }
              }}>
