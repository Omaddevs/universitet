...
export default function EventsSection() {
  ...
  return <motion.section className="evx" variants={wrap} initial="hidden" whileInView="show" viewport={{
    once: true,
    amount: 0.18
  }}>
      <div className="evx-container">
        <motion.div className="evx-head" variants={fadeDown}>
          <h2 className="evx-title">{t("upcoming")}<span>{t("events")}</span>
          </h2>

          <a className="evx-more" href="/">{t("see_more")}<IoChevronForwardOutline />
          </a>
        </motion.div>

        <div className="evx-trackWrap">
          <motion.div className="evx-track" role="list" variants={wrap}>
            {events.map((e, idx) => <motion.article className="evx-card" key={e.id} role="listitem" variants={idx % 2 === 0 ? cardFromLeft : cardFromRight}>
