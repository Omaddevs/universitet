...
export default function DepartmentCard({
  ...
  return <article className="sdc-card">
      <Link className="sdc-link" to={`/staff/${department.id}`} aria-label={`Open ${department.title}`}>
        ...
        <div className="sdc-bottom">
          <div className="sdc-avatars" aria-hidden="true">
            {preview.map(member => <img key={member.name} src={member.photoUrl} alt="" className="sdc-avatar" loading="lazy" />)}
          </div>

          <span className="sdc-more">{t("see_more")}<IoChevronForwardOutline />
          </span>
        </div>
      </Link>
    </article>;
